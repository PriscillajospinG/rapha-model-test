"""
automation/run_upper_pipeline.py
================================
One-command automation for the Upper Limb CTR-GCN pipeline.

Pipeline steps:
    1. preprocessing/extract_upper_limb_dataset.py   — MediaPipe pose extraction
    2. preprocessing/build_upper_ctrgcn_dataset.py    — Build skeleton tensors
    3. preprocessing/split_upper_dataset.py           — Train / test split
    4. training/train_upper_limb_ctrgcn.py            — Model training

Usage:
    python automation/run_upper_pipeline.py
    python automation/run_upper_pipeline.py --skip-extraction
    python automation/run_upper_pipeline.py --only-train
    python automation/run_upper_pipeline.py --skip-training
...
Logs saved to: logs/upper_pipeline.log
"""
...
PIPELINE_NAME = "Upper Limb"

STEPS: list[dict] = [
    {
        "number": 1,
        "description": "Extracting Upper Limb Dataset  (MediaPipe Pose)",
        "script": PROJECT_ROOT / "preprocessing" / "extract_upper_limb_dataset.py",
    },
    {
        "number": 2,
        "description": "Building Upper Limb CTR-GCN Skeleton Tensors",
        "script": PROJECT_ROOT / "preprocessing" / "build_upper_ctrgcn_dataset.py",
    },
    {
        "number": 3,
        "description": "Splitting Upper Limb Dataset into Train / Test Sets",
        "script": PROJECT_ROOT / "preprocessing" / "split_upper_dataset.py",
    },
    {
        "number": 4,
        "description": "Training Upper Limb CTR-GCN Model",
        "script": PROJECT_ROOT / "training" / "train_upper_limb_ctrgcn.py",
    },
]

SKELETONS_DIR = PROJECT_ROOT / "processed_dataset_upper" / "skeletons"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="run_upper_pipeline.py",
        description="One-command automation for the Upper Limb CTR-GCN pipeline.",
    )
...
def main() -> int:
    """
    Entry point for the Upper Limb pipeline.
    ...
    logger = setup_logger(LOG_FILE, logger_name="upper_pipeline")
    log_message(logger, "info", f"{'='*60}")
    log_message(logger, "info", f"Upper Limb Pipeline started — {time.strftime('%Y-%m-%d %H:%M:%S')}")
