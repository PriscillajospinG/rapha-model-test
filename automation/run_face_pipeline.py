"""
automation/run_face_pipeline.py
================================
One-command automation for the Face CTR-GCN pipeline.

Pipeline steps:
    1. preprocessing/extract_face_dataset.py       — MediaPipe face landmark extraction
    2. preprocessing/build_face_ctrgcn_dataset.py  — Build skeleton tensors
    3. preprocessing/split_face_dataset.py         — Train / test split
    4. training/train_face_ctrgcn.py               — Model training

Usage:
    python automation/run_face_pipeline.py
    python automation/run_face_pipeline.py --skip-extraction
    python automation/run_face_pipeline.py --only-train
    python automation/run_face_pipeline.py --skip-training
...
Logs saved to: logs/face_pipeline.log
"""
...
STEPS: list[dict] = [
    {
        "number": 1,
        "description": "Extracting Face Dataset  (MediaPipe Face Landmarks)",
        "script": PROJECT_ROOT / "preprocessing" / "extract_face_dataset.py",
    },
    {
        "number": 2,
        "description": "Building Face CTR-GCN Skeleton Tensors",
        "script": PROJECT_ROOT / "preprocessing" / "build_face_ctrgcn_dataset.py",
    },
    {
        "number": 3,
        "description": "Splitting Face Dataset into Train / Test Sets",
        "script": PROJECT_ROOT / "preprocessing" / "split_face_dataset.py",
    },
    {
        "number": 4,
        "description": "Training Face CTR-GCN Model",
        "script": PROJECT_ROOT / "training" / "train_face_ctrgcn.py",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="run_face_pipeline.py",
        description="One-command automation for the Face CTR-GCN pipeline.",
    )
...
def main() -> int:
    """
    Entry point for the Face pipeline.
    ...
    logger = setup_logger(LOG_FILE, logger_name="face_pipeline")
    log_message(logger, "info", f"{'='*60}")
    log_message(logger, "info", f"Face Pipeline started — {time.strftime('%Y-%m-%d %H:%M:%S')}")
