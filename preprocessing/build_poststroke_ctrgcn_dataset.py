...
Tensor layout:
    ...
    V=12 : post-stroke joints (nodes 0-11)
...
EXPECTED_V = 12   # post-stroke joints
EXPECTED_M = 1    # person

POSTSTROKE_NODES = list(range(EXPECTED_V))  # nodes 0..11


def build_tensors() -> None:
    if not INPUT_CSV.exists():
        log.error(
            "Input CSV not found at %s. Please run extract_poststroke_dataset.py first.",
            INPUT_CSV,
        )
        sys.exit(1)
