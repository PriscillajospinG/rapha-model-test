"""
preprocessing/extract_upper_limb_dataset.py
───────────────────────────────────────────────────────────────────────────
...
UPPER_LIMB_JOINTS_REMAP  = list(range(8))                      # Graph nodes 0..7
...
JOINT_COLS = []
for _node in UPPER_LIMB_JOINTS_REMAP:
    JOINT_COLS += [
        f"joint_{_node}_x",
        f"joint_{_node}_y",
        f"joint_{_node}_z",
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(BASE_DIR / "upper_extraction.log", encoding="utf-8"),
    ],
)
    ...
    with mp_vision.PoseLandmarker.create_from_options(options) as landmarker:
        while True:
            ...
            if result.pose_landmarks and len(result.pose_landmarks) > 0:
                ...
                for node_idx, mp_id in enumerate(UPPER_LIMB_JOINTS_MP):
                    lm = lm_list[mp_id]
                ...
                failed += 1
                for node_idx in UPPER_LIMB_JOINTS_REMAP:
...
def main() -> None:
    logger.info("=" * 70)
    logger.info("  Physiotherapy Upper-Limb Dataset Extraction Pipeline")
    logger.info("=" * 70)
