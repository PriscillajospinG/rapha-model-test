...
def split() -> None:
    if not SKELETON_DIR.exists():
        log.error(
            ...
            "Please run build_upper_ctrgcn_dataset.py first.",
            SKELETON_DIR,
        )
        sys.exit(1)
    ...
    # The CSV contains (video_name, frame, label, ...) where label is the
    # class string. Build a dynamic map: class_name → integer_id
    ...
    class_map_path = OUTPUT_DIR / "upper_class_map.csv"
    pd.DataFrame(
