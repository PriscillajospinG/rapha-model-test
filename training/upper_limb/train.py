"""
training/train_upper_limb_ctrgcn.py
───────────────────────────────────────────────────────────────────────────
CTR-GCN training pipeline for upper-limb physiotherapy exercise recognition.

Pre-flight validation (always runs before any gradient step):
  ① Verify all tensors in train + test are (4, 300, 8, 1)
  ② Print the custom upper-limb graph connections and adjacency info
...
from preprocessing.upper_limb.loader import (
    EXPECTED_C, EXPECTED_T, EXPECTED_V, EXPECTED_M,
    UpperPhysioDataset,
    build_upper_loaders,
    load_class_map,
)
from preprocessing.upper_limb.graph import UpperLimbGraph
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(BASE_DIR / "upper_training.log", encoding="utf-8"),
    ],
)
...
def validate_tensors() -> None:
    ...
    log.info("── [Validation 1/4] Tensor shapes ──────────────────────────────")
...
def validate_graph(graph: UpperLimbGraph) -> None:
    """Print graph info and assert adjacency properties."""
    log.info("── [Validation 2/4] Graph adjacency ────────────────────────────")
    graph.print_info()

    A = graph.A
    assert A.shape == (3, 8, 8), f"Expected A shape (3,8,8), got {A.shape}"
...
def validate_classes(
    train_ds: UpperPhysioDataset,
    class_names: dict[int, str],
    num_classes: int,
) -> None:
    """Ensure all expected classes are in the training split."""
    log.info("── [Validation 3/4] Class coverage ─────────────────────────────")
...
def train_one_epoch(
    ...
    """Run one full training epoch. Supports optional CUDA AMP via *scaler*.
...
def plot_curves(
    ...
    ax.set_title("Upper-Limb CTR-GCN — Training Loss"); ax.legend()
    plt.tight_layout()
    ...
    ax.set_title("Upper-Limb CTR-GCN — Training vs. Validation Accuracy"); ax.legend()
    plt.tight_layout()
...
def plot_confusion_matrix(
    ...
    ax.set_title("CTR-GCN Upper-Limb — Confusion Matrix (Test Set)", fontsize=14)
...
def main() -> None:
    ...
    log.info("=" * 65)
    log.info("  CTR-GCN Upper-Limb Training Pipeline")
    log.info("=" * 65)
    ...
    class_names = load_class_map(str(CLASS_MAP_CSV))   # {int → str}
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    log.info("\n%s", "=" * 65)
    log.info("  Upper-Limb Training Complete")
    log.info("%s", "=" * 65)
