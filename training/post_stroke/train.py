"""
training/train_poststroke_ctrgcn.py
───────────────────────────────────────────────────────────────────────────
CTR-GCN training pipeline for post-stroke physiotherapy exercise recognition.

Pre-flight validation (always runs before any gradient step):
  ① Verify all tensors in train + test are (4, 300, 12, 1)
  ② Print the custom post-stroke graph connections and adjacency info
  ③ Confirm all exercise classes are present in the training set
  ④ Verify no train/test overlap (i.e. disjoint datasets)
...
Training configuration:
  ...
  Batch size : 8
  Epochs     : 100
  Augment    : noise + temporal-flip + left-right mirror (training only)
...
from preprocessing.post_stroke.loader import (
    EXPECTED_C, EXPECTED_T, EXPECTED_V, EXPECTED_M,
    PostStrokePhysioDataset,
    build_poststroke_loaders,
    load_class_map,
)
from preprocessing.post_stroke.graph import PostStrokeGraph
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(BASE_DIR / "poststroke_training.log", encoding="utf-8"),
    ],
)
...
def validate_tensors() -> None:
    ...
    log.info(
        "  ✓  All %d tensors verified as %s",
        len(all_names), expected,
    )


def validate_graph(graph: PostStrokeGraph) -> None:
    """Print graph info and assert adjacency properties."""
    log.info("── [Validation 2/5] Graph adjacency ────────────────────────────")
    graph.print_info()

    A = graph.A
    assert A.shape == (3, 12, 12), f"Expected A shape (3,12,12), got {A.shape}"
...
def validate_classes(
    train_ds: PostStrokePhysioDataset,
    class_names: dict[int, str],
    num_classes: int,
) -> None:
    """Ensure all expected classes are in the training split."""
    ...
    for cls_id in sorted(dist):
        ...
        log.info("    Class %d  %-22s  %d sample(s)", cls_id, cls_name, dist[cls_id])
...
def validate_no_overlap() -> None:
    """Verify that there is no sample overlap between train and test datasets."""
    log.info("── [Validation 4/5] Train/Test split overlap ───────────────────")
    ...
    if overlap:
        log.error("  Train/test overlap detected! Overlapping samples: %s", overlap)
        sys.exit(1)
    else:
        log.info("  ✓  Zero overlap between train and test datasets.")
...
def validate_forward(
    ...
    log.info("── [Validation 5/5] Model forward pass ─────────────────────────")
    model.eval()
...
def plot_curves(
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("Post-Stroke CTR-GCN — Training Loss")
    ax.legend()
    plt.tight_layout()
    ...
    # Accuracy curve
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(epochs, [a * 100 for a in train_accs], color="#3498db", linewidth=2, label="Train acc")
    ax.plot(epochs, [a * 100 for a in val_accs], color="#2ecc71", linewidth=2, linestyle="--", label="Val acc")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Accuracy (%)")
    ax.set_title("Post-Stroke CTR-GCN — Training vs. Validation Accuracy")
    ax.legend()
    plt.tight_layout()
...
def plot_confusion_matrix(
    ...
    fig, ax = plt.subplots(figsize=(max(8, num_classes + 2), max(7, num_classes + 1)))
    sns.heatmap(
    ...
    ax.set_title("CTR-GCN Post-Stroke — Confusion Matrix (Test Set)", fontsize=14)
...
def main() -> None:
    ...
    log.info("=" * 65)
    log.info("  CTR-GCN Post-Stroke Training Pipeline")
    log.info("=" * 65)
    ...
    effective_workers = min(8, os.cpu_count() or 1) if device.type == "cuda" else NUM_WORKERS
    ...
    train_ds, test_ds, train_loader, test_loader = build_poststroke_loaders(
    ...
    # ── Training loop ─────────────────────────────────────────────────────────
    log.info("Training  —  %d epochs  |  batch=%d  |  lr=%.4f  |  classes=%d",
    ...
    # Save classification report to txt file as requested
    report_file = RESULTS_DIR / "classification_report.txt"
    with open(report_file, "w", encoding="utf-8") as rf:
        rf.write(report)
    log.info("  Classification report saved: classification_report.txt")

    # ── Summary ───────────────────────────────────────────────────────────────
    log.info("\n%s", "=" * 65)
    log.info("  Post-Stroke Training Complete")
    log.info("%s", "=" * 65)
